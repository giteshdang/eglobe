...
import { BackgroundServiceSvg, OutlineDashStaff, TranslationSvg } from "../../assets/svg"
        ...
        <div className="flex md:px-24 flex-col space-y-24 py-8 md:py-24 px-8">
            <div className="grid-cols-1 md:grid-cols-2 2xl:grid-cols-2 grid gap-16 xl:gap-40">
                <div className="grid grid-cols-1 col-span-1 md:col-span-2 md:grid-cols-2 2xl:col-span-1 2xl:grid-cols-1  gap-8 md:gap-24">
                    ...
                    <img src={OutlineDashStaff} className="w-40 h-6 object-fill" alt=""  data-aos='fade-right' data-aos-duration='3000' />
                        <h1 className="tahoma-bold text-4xl text-white" data-aos='fade-right' data-aos-duration='3000'>
                ...
                <Marquee key={key}
                    ...
                    minScale={0.7}>
                    {
                        times(6, Number).map((val, index)=>{
                            return (
                                <div onMouseOver={()=>onHover()} onMouseLeave={()=>onMouseLeave()} key={index} className="flex flex-row w-96 h-12 items-center justify-center space-x-2 transition ease-in-out cursor-pointer hover:-translate-y-2 hover:scale-150 duration-3000">
                                    <img src={TranslationSvg} className="w-8 h-8"  alt="" />
                                    <div className="tahoma-bold text-lg text-eglobe-orange">{data[index].desc}</div>
                                    </div>
                            )
                        })
                    }
